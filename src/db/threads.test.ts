...
describe.skipIf(!HAS_DB)("createThread + getThreadMeta", () => {
  test("creates a thread", async () => {
    const id = await createThread(principalId);
  ...
  test("creates a titled thread", async () => {
    const id = await createThread(principalId, "Test Subject");
    const meta = await getThreadMeta(id, principalId);
...
describe.skipIf(!HAS_DB)("recentThreads", () => {
  test("returns threads ordered by latest activity", async () => {
    const t1 = await createThread(principalId);
    await insertMessage({ content: "Old", role: "user", threadId: t1 });

    await Bun.sleep(10);
    const t2 = await createThread(principalId);
  ...
  test("scope=mine returns threads with chat messages", async () => {
    const chat = await createThread(principalId);
    await insertMessage({ content: "Hi", role: "user", threadId: chat });

    const email = await createThread(principalId, "Test");
    await insertMessage({ content: "Hi", metadata: { from: "a@b.com" }, role: "user", threadId: email });

    const mine = await recentThreads(principalId, { scope: "mine" });
    expect(mine.some((t) => t.id === chat)).toBe(true);
    expect(mine.some((t) => t.id === email)).toBe(false);
  });

  test("includes snippet from latest message", async () => {
    const t = await createThread(principalId);
...
describe.skipIf(!HAS_DB)("findEmailThread", () => {
  test("finds thread by message reference", async () => {
    const t = await createThread(principalId, "Test");
    await insertMessage({
      content: "Hi",
  ...
  test("falls back to subject matching when thread has email metadata", async () => {
    const t = await createThread(principalId, "Project Update");
    await insertMessage({
      content: "Hi",
