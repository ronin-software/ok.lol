...
const emailReceive: Capability<OriginExecutionContext, GetReceivingEmailResponseSuccess, void> = {
  ...
  async call(ectx, email) {
    ...
    const prompt = [
      "You received an email. Read it carefully and reply appropriately if needed.",
      "If you take an action on a received email (other than spam), reply back to let the sender know.",
      "Be mindful of whether the email was sent by me or someone else.",
      "You can email me to let me know when you receive an email and it warrants my attention,",
      "and you can reply to conversational emails sent by others.",
      "",
      `From: ${email.from}`,
      `Subject: ${subject}`,
      "",
